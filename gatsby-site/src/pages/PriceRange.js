        ...
        cards.map( card => {
          return (
            <Card key={card.title} style={{ width: '18rem', margin: '2rem', borderRadius: '8px'}}>
              <Card.Img onMouseOver = {() => console.log('hov')} src={card.img} style={{ width: '100%', height: '60%', objectFit: 'contain', padding: '10px'}} />
              <Card.Body style={{  borderRadius: '8px' }}>
              <Card.Title style={{ color: '#000'}}>{card.title}</Card.Title>
              <Card.Text style={{ fontWeight: 'bold'}} >KSH {card.range}</Card.Text>
                <Link to="/PriceRange">
